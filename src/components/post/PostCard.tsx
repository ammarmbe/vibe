...
export default function PostCard({ post }: { post: Post }) {
  const commentOrComments = post.comments == 1 ? "Comment" : "Comments";
    ...
    <article className="border rounded-md p-2.5 gap-1.5 flex shadow-sm">
      ...
      <div className="flex flex-col flex-grow">
        <div className="flex justify-between items-baseline w-full">
          <h2 className="leading-none font-medium">{post.name}</h2>
          <p className="text-sm leading-none text-black/70">
            {dayjs(new Date(post.createdAt * 1000)).fromNow()}
          </p>
        </div>
        <a
        ...
        <div className="flex gap-1.5">
          <LikeButton
            count={post.likes}
            liked={post.likedByUser == 0 ? false : true}
            postId={post.postId}
          />
          <a
